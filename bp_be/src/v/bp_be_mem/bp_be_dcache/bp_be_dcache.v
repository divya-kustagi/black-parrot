...
module bp_be_dcache
 ...
 #(parameter bp_params_e bp_params_p = e_bp_inv_cfg
  ...
  bp_be_dcache_tag_info_s [d_lce_assoc_p-1:0] tag_mem_data_li;
  bp_be_dcache_tag_info_s [d_lce_assoc_p-1:0] tag_mem_mask_li;
  bp_be_dcache_tag_info_s [d_lce_assoc_p-1:0] tag_mem_data_lo;
  
  bsg_mem_1rw_sync_mask_write_bit
    #(.width_p(tag_info_width_lp*d_lce_assoc_p)
      ,.els_p(d_lce_sets_p)
    )
    tag_mem
      (.clk_i(clk_i)
  ...
  // data_mem
  //
  logic [d_lce_assoc_p-1:0] data_mem_v_li;
  logic data_mem_w_li;
  logic [d_lce_assoc_p-1:0][index_width_lp+word_offset_width_lp-1:0] data_mem_addr_li;
  logic [d_lce_assoc_p-1:0][(cce_block_width_p/d_lce_assoc_p)-1:0] data_mem_data_li;
  logic [d_lce_assoc_p-1:0][data_mask_width_lp-1:0] data_mem_mask_li;
  logic [d_lce_assoc_p-1:0][(cce_block_width_p/d_lce_assoc_p)-1:0] data_mem_data_lo;
  
  for (genvar i = 0; i < d_lce_assoc_p; i++) begin: data_mem
    bsg_mem_1rw_sync_mask_write_byte
      #(.data_width_p(cce_block_width_p/d_lce_assoc_p)
        ,.els_p(d_lce_sets_p*d_lce_assoc_p)
        )
      data_mem
        (.clk_i(clk_i)
        ,.reset_i(reset_i)
  ...
  bp_be_dcache_tag_info_s [d_lce_assoc_p-1:0] tag_info_tv_r;
  logic [d_lce_assoc_p-1:0][(cce_block_width_p/d_lce_assoc_p)-1:0] ld_data_tv_r;
  logic [ptag_width_lp-1:0] addr_tag_tv;
  logic [index_width_lp-1:0] addr_index_tv;
  logic [word_offset_width_lp-1:0] addr_word_offset_tv;

  localparam byte_offset_msb_lp=`BSG_SAFE_CLOG2(data_mask_width_lp >> 3);
  logic [byte_offset_msb_lp-1:0] addr_byte_offset_msb_tv;
  ...
  assign addr_byte_offset_msb_tv = paddr_tv_r[3+:byte_offset_msb_lp];

  // miss_detect
  //
  logic [d_lce_assoc_p-1:0] tag_match_tv;
  logic [d_lce_assoc_p-1:0] load_hit_tv;
  logic [d_lce_assoc_p-1:0] store_hit_tv;
  logic [d_lce_assoc_p-1:0] invalid_tv;
  logic load_hit;
  logic store_hit;
  logic [way_id_width_lp-1:0] load_hit_way;
  logic [way_id_width_lp-1:0] store_hit_way;

  for (genvar i = 0; i < d_lce_assoc_p; i++) begin: tag_comp
  ...
  // write buffer
  //
  `declare_bp_be_dcache_wbuf_entry_s(paddr_width_p, dword_width_p, d_lce_assoc_p);
  ...
  logic [d_lce_assoc_p-1:0][cce_block_width_p/d_lce_assoc_p-1:0] lce_data_mem_data_li;
  ...
  assign miss_tracker_en_li = cache_req_v_o && ~uncached_store_req;
  ...
  assign cache_miss = (cache_req_v_o & ~uncached_store_req) || miss_tracker_r;
  ...
  logic [(cce_block_width_p/d_lce_assoc_p)-1:0] ld_data_way_picked;
  logic [dword_width_p-1:0] ld_data_way_picked_word;
  logic [dword_width_p-1:0] bypass_data_masked;

  bsg_mux #(
    .width_p(cce_block_width_p/d_lce_assoc_p)
    ,.els_p(d_lce_assoc_p)
  ...
  bsg_mux #(
    ...
    ,.els_p(8/d_lce_assoc_p)// replace 8 with max assoc
  ) ld_data_set_select_mux_word (
    .data_i(ld_data_way_picked)
    ,.sel_i(addr_byte_offset_msb_tv)
    ,.data_o(ld_data_way_picked_word)
  );
  ...
  bsg_mux_segmented #(
    ...
    .data0_i(ld_data_way_picked_word)
  ...
  // data_mem
  //
  logic [d_lce_assoc_p-1:0] wbuf_data_mem_v;
  bsg_decode #(
    .num_out_p(d_lce_assoc_p)
  ...
  assign data_mem_v_li = (load_op & tl_we)
    ...
    : (wbuf_yumi_li
      ? wbuf_data_mem_v
      : {d_lce_assoc_p{lce_data_mem_v}});

  assign data_mem_w_li = wbuf_yumi_li
    | (data_mem_pkt_v_i & data_mem_pkt.opcode == e_cache_data_mem_write);

  logic [d_lce_assoc_p-1:0][(cce_block_width_p/d_lce_assoc_p)-1:0] lce_data_mem_write_data;

  for (genvar i = 0; i < d_lce_assoc_p; i++) begin
  ...
  logic [d_lce_assoc_p-1:0] lce_tag_mem_way_one_hot;
  bsg_decode
    #(.num_out_p(d_lce_assoc_p))
  ...
  always_comb begin
    case (tag_mem_pkt.opcode)
      e_cache_tag_mem_set_clear: begin
        tag_mem_data_li = {(tag_info_width_lp*d_lce_assoc_p){1'b0}};
        tag_mem_mask_li = {(tag_info_width_lp*d_lce_assoc_p){1'b1}};
      end
      e_cache_tag_mem_invalidate: begin
        tag_mem_data_li = {((tag_info_width_lp)*d_lce_assoc_p){1'b0}};
        for (integer i = 0; i < d_lce_assoc_p; i++) begin 
          tag_mem_mask_li[i].coh_state = {`bp_coh_bits{lce_tag_mem_way_one_hot[i]}};
          tag_mem_mask_li[i].tag = {ptag_width_lp{1'b0}};
        end
      end
      e_cache_tag_mem_set_tag: begin
        tag_mem_data_li = {d_lce_assoc_p{tag_mem_pkt.state, tag_mem_pkt.tag}};
        for (integer i = 0; i < d_lce_assoc_p; i++) begin
          tag_mem_mask_li[i].coh_state = {`bp_coh_bits{lce_tag_mem_way_one_hot[i]}};
      ...
      default: begin
        tag_mem_data_li = {(tag_info_width_lp*d_lce_assoc_p){1'b0}};
        tag_mem_mask_li = {(tag_info_width_lp*d_lce_assoc_p){1'b0}};
      end
    endcase
  end
  ...
  logic [d_lce_assoc_p-2:0] lru_decode_data_lo;
  logic [d_lce_assoc_p-2:0] lru_decode_mask_lo;
  ...
  logic [d_lce_assoc_p-1:0] dirty_mask_lo;
  ...
  always_comb begin
    if (v_tv_r) begin
      ...
      stat_mem_data_li.dirty = {d_lce_assoc_p{1'b1}};
      ...
      case (stat_mem_pkt.opcode)
        ...
        e_cache_stat_mem_clear_dirty: begin
          stat_mem_data_li = {(stat_info_width_lp){1'b0}};
          stat_mem_mask_li.lru = {(d_lce_assoc_p-1){1'b0}};
  ...
  initial begin
    ...
    //assert(d_lce_assoc_p == 8) else $error("d_lce_assoc_p has to be 8");
  end
